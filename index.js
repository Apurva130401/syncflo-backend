...
app.post('/api/webhooks/nango', async (req, res) => {
  ...
  if (webhook.type === 'auth' && webhook.operation === 'creation' && webhook.success) {
    try {
      ...
      // This is the User ID you passed during the nango.auth() call
      const userId = webhook.connectionId; 
      
      // This is the *new* unique connection ID generated by Nango
      const nangoConnectionId = webhook.model.connection_id; 
      
      const provider = webhook.providerConfigKey; // Use providerConfigKey for consistency

      console.log(`Received new connection: User [${userId}] connected [${provider}] with new Nango ID [${nangoConnectionId}]`);
      ...
      if (columnToUpdate) {
        ...
        // Save the NEW Nango ID for the correct user
