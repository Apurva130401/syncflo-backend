...
app.delete('/api/connections', async (req, res) => {
    ...
    try {
        ...
        console.error(`Error deleting connection ${connectionId} for provider ${providerConfigKey}:`, err);
        res.status(500).json({ error: 'Failed to delete connection.', details: err && (err.message || err.toString()), stack: err && err.stack });
    }
});
